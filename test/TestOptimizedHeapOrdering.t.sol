...
import "src/OptimizedHeapOrdering.sol";
import "test/commons/Test2Heap.sol";
import "test/commons/IHeapWrapper.sol";

contract OptimizedHeapOrderingWrapper is IHeapWrapper {
    using OptimizedHeapOrdering for OptimizedHeapOrdering.HeapArray;
    OptimizedHeapOrdering.HeapArray heap;
    ...
    function accounts(uint256 _index) external view returns (address, uint256) {
        OptimizedHeapOrdering.Account memory account = heap.accounts[_index];
...
contract TestOptimizedHeapOrdering is Test2Heap {

    constructor () {
        wrapper = new OptimizedHeapOrderingWrapper();
    }
}
